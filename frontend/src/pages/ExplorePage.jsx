...
export const ExplorePage = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener("actualizarCatalogo", recargarCatalogo);

    // Limpieza del evento cuando salimos de la página
    return () => {
      window.removeEventListener("actualizarCatalogo", recargarCatalogo);
    };
  }, [searchString]);
    ...
    try {
      ...
      if (res.ok) {
        if (data.isFavorite) {
          ...
          if (tipo === "comercio")
            setFavComercios((prev) => prev.filter((item) => item !== id));
        }
      }
    } catch (error) {
  ...
  //  Creamos un "diccionario(set)" rápido con los IDs de las tiendas que SÍ tienen productos
  const comerciosConProductos = new Set(products.map((p) => p.id_comercio));

  const filteredShops = shops.filter((shop) => {
    // Comprobamos que el el set contiene un producto como minimo (true/false)
    const tieneProductos = comerciosConProductos.has(shop.id_comercio);

    const matchCategoria =
      selectedCategory === "Todas" || shop.categoria === selectedCategory;
    const matchBusqueda = shop.nombre
      .toLowerCase()
      .includes(searchQuery.toLowerCase());

    return tieneProductos && matchCategoria && matchBusqueda;
  });

  const filteredProducts = products.filter((product) => {
    const matchCategoria =
      selectedCategory === "Todas" || product.categoria === selectedCategory;
    const matchBusqueda = product.nombre
      .toLowerCase()
      .includes(searchQuery.toLowerCase());
    ...
    <div className="relative overflow-hidden min-h-screen">
      ...
      <div className="flex flex-col md:flex-row gap-8 max-w-7xl mx-auto px-4 py-8 w-full relative z-0">
        ...
        <aside className="w-full md:w-64 shrink-0">
          <h2 className="font-bold text-xl mb-4 text-base-content">
            Categorías
          </h2>
          ...
          <header className="mb-10">
            <div className="relative group w-full mb-8">
              ...
              <div className="relative flex items-center bg-base-100/80 backdrop-blur-md rounded-full shadow-sm border border-base-300 overflow-hidden">
                <div className="pl-5 text-base-content/40">
                  <Search size={20} />
                </div>
                <input
                  type="text"
            ...
            <div className="flex gap-6 border-b border-base-200">
              <button
                ...
                className={`pb-3 px-2 font-bold border-b-2 flex items-center gap-2 ${viewMode === "tiendas" ? "border-jungle_teal text-jungle_teal" : "border-transparent opacity-50 cursor-pointer"}`}
              >
                <Store size={18} /> Tiendas
              </button>
              <button
                onClick={() => setViewMode("productos")}
                className={`pb-3 px-2 font-bold border-b-2 flex items-center gap-2 ${viewMode === "productos" ? "border-jungle_teal text-jungle_teal" : "border-transparent opacity-50 cursor-pointer"}`}
              >
                <ShoppingBasket size={18} /> Productos
              </button>
              <button
                onClick={() => setViewMode("todos")}
                className={`pb-3 px-2 font-bold border-b-2 flex items-center gap-2 ${viewMode === "todos" ? "border-jungle_teal text-jungle_teal" : "border-transparent opacity-50 cursor-pointer"}`}
              >
          ...
          <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                      ...
                      <div className="card bg-base-100/80 backdrop-blur-md shadow-sm border border-base-200 hover:shadow-md transition-all rounded-3xl cursor-pointer h-full">
                        ...
                        <figure className="h-48 bg-base-200 overflow-hidden rounded-t-3xl relative">
                          ...
                          <button
                            ...
                            className={`absolute top-2 right-2 p-2 backdrop-blur-sm rounded-full transition-all shadow-md z-10 cursor-pointer ${
                              favComercios.includes(shop.id_comercio)
                        ...
                        <div className="card-body p-4">
                          ...
                          <p className="text-xs font-bold text-jungle_teal">
                            {shop.categoria}
                          </p>
                ...
                {(viewMode === "productos" || viewMode === "todos") &&
                  filteredProducts.map((product) => (
                    <div
                      key={`prod-${product.id_producto}`}
                      className="card bg-base-100/80 backdrop-blur-md shadow-sm border border-base-200 hover:shadow-md transition-all rounded-3xl flex flex-col"
                    >
                      <figure className="h-48 bg-base-200 overflow-hidden rounded-t-3xl relative shrink-0">
                        ...
                        <button
                          ...
                          className={`absolute top-2 right-2 p-2 backdrop-blur-sm rounded-full transition-all shadow-md z-10 cursor-pointer ${
                            favProductos.includes(product.id_producto)
                              ? "text-red-500 bg-white"
                              : "text-base-content/50 bg-base-100/80 hover:text-red-400"
                          }`}
                        >
                      ...
                      <div className="card-body p-4 flex-1 flex flex-col justify-between">
                        ...
                        <div className="flex justify-between items-center mt-4">
                          <span className="font-black text-jungle_teal text-xl">
                            {product.precio}€
                          </span>
                          {/* ✨ LÓGICA DE STOCK (BOTÓN) ✨ */}
                          <button
                            onClick={() => handleAddToCart(product)}
                            disabled={product.stock <= 0}
                            className="btn btn-sm bg-jungle_teal text-white border-none rounded-xl px-6 hover:bg-jungle_teal/80 cursor-pointer shadow-md shadow-jungle_teal/20 disabled:bg-base-300 disabled:text-base-content/40 disabled:shadow-none"
                          >
            ...
            {sinResultados && (
              <div className="flex flex-col items-center justify-center text-center p-10 bg-base-100/80 backdrop-blur-md rounded-4xl border border-base-300 shadow-sm min-h-[50vh]">
                ...
                <h3 className="font-black text-2xl text-base-content mb-3">
                  ¡Vaya, parece que el mercado está muy tranquilo hoy!
                </h3>
                <p className="text-base-content/70 max-w-md mb-8">
                  No hemos encontrado ninguna tienda ni producto que coincida
                  con tu búsqueda de "{searchQuery || selectedCategory}". Prueba
                  a cambiar la categoría o busca otra cosa.
                </p>
                <button
                  onClick={() => {
                    setSearchQuery("");
